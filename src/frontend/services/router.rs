...
pub trait Routable {
    ...
    fn tune_from_part(route_section: RouteSection) -> Option<Self>
    where
        Self: Sized;
}
...
impl RouteInfo {
    ...
    pub fn parse(route_string: &str) -> Result<RouteInfo, RoutingError> {
        ...
        Url::parse(&full_url)
            .map(RouteInfo::from)
            .map_err(|_| RoutingError::CouldNotParseRoute {
...
impl RouteService {
    ...
    pub fn new() -> RouteService {
        RouteService {
            ...
            location: window()
                .location()
                .expect("Could not find Location API, routing is not supported in your browser."),
            event_listener: None,
            callback: None,
        }
    }
    ...
    pub fn go_to_current_route(&mut self) {
        if let Some(ref cb) = self.callback {
            let route_result: RouteResult = match self.get_location() {
                Ok(full_url) => Url::parse(&full_url).map(RouteInfo::from).map_err(|_| {
                    RoutingError::CouldNotParseUrl {
